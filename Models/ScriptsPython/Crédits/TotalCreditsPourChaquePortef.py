...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"Portefeuille: 0{test_data[i][0]}, totalCreditDEB:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'Portefeuille', 'totalCreditDEB' et 'DATE'
...
# #Tracé regression liniére simple
# # Tracer le nuage de points des données de base avec des marqueurs rouges ('o')
# plt.scatter(X[:, 0], y, color='red', label='Données', marker='o')

# # Tracer les prédictions avec des marqueurs bleus ('^')
# plt.scatter(test_data[:, 0], y_pred, color='blue', label='Prédictions', marker='^')

# # Tracer la droite de régression
# plt.plot(X[:, 0], y, color='green', label='Régression linéaire')

# # Étiquettes des axes
# plt.xlabel('Portefeuille')
# plt.ylabel('Total Crédit')

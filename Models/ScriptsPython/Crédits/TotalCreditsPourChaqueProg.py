...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"Programme: 0{test_data[i][0]}, totalCreditDEB:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'Programme', 'totalCreditDEB' et 'DATE'
    for column in columns:
        if 'Programme' in column:
...
test_data = np.array(test_data) # Nouvelles données : Programme, date qui représente le X_test
...
#     plot_tree(regressor_tree, feature_names=['Programme', 'Date'], filled=True)
#     plt.show()

...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"SousProgramme: 0{test_data[i][0]}, totalCreditDEB:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'SousProgramme', 'totalCreditDEB' et 'DATE'
    for column in columns:
        if 'SousProgramme' in column:
            value = int(column.split(':')[1].strip())  # Extraire et convertir en entier
            column_values.append(value)
        elif 'totalCreditDEB' in column:
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (SousProgramme) et la deuxième colonne (date)
    SousProgramme = row[0] 
    ...
    test_data.append([SousProgramme, date])



# Convertir test_data en tableau numpy
test_data = np.array(test_data) # Nouvelles données : SousProgramme, date qui représente le X_test
...
#     scene=dict(
#         xaxis_title='SousProgramme',
#         yaxis_title='Date',
#         zaxis_title='Somme Crédits'
#     )
# )
...
#     plot_tree(regressor_tree, feature_names=['SousProgramme', 'Date'], filled=True)
#     plt.show()
...
# # Plot with zoom-out effect (smaller figure size)
# plot_tree_with_size(30, 30)  # Zoom-out

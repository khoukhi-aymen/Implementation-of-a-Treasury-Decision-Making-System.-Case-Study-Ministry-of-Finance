...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"Titre: T{test_data[i][0]}, totalCreditDEB:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'Titre', 'totalCreditDEB' et 'DATE'
    for column in columns:
        if 'Titre' in column:
            value = int(re.findall(r'\d+', column.split(':')[1].strip())[0])  # Extraire et convertir en entier
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (Titre) et la deuxième colonne (date)
    Titre = row[0] 
    ...
    test_data.append([Titre, date])



# Convertir test_data en tableau numpy
test_data = np.array(test_data) # Nouvelles données : Titre, date qui représente le X_test
...
#     plot_tree(regressor_tree, feature_names=['Titre', 'Date'], filled=True)
#     plt.show()

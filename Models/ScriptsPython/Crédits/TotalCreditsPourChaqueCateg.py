...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"Categorie: {test_data[i][0]}, totalCreditDEB:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'Categorie', 'totalCreditDEB' et 'DATE'
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (Categorie) et la deuxième colonne (date)
    Categorie = row[0] 
    ...
    test_data.append([Categorie, date])



# Convertir test_data en tableau numpy
test_data = np.array(test_data) # Nouvelles données : Categorie, date qui représente le X_test
...
#     plot_tree(regressor_tree, feature_names=['Categorie', 'Date'], filled=True)
#     plt.show()

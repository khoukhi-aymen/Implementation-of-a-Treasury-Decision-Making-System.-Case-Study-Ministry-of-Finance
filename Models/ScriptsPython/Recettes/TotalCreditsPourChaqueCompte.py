...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"CodeCompte: {test_data[i][0]}, TotalMontantRecette:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
test_data = np.array(test_data)  # Nouvelles données : CodeCompte, date qui représente le X_test
...
#     plot_tree(regressor_tree, feature_names=['CodeCompte', 'Date'], filled=True)
#     plt.show()

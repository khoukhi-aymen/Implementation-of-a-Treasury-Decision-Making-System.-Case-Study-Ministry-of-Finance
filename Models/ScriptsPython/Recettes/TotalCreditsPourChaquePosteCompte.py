...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"CodeWilaya: {test_data[i][0]}, TotalMontantRecette:  {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'Categorie', 'TotalMontantRecette' et 'DATE'
    for column in columns:
        if 'CodeWilaya' in column:
            value = int(column.split(':')[1].strip())  # Extraire et convertir en entier
            column_values.append(value)
        elif 'TotalMontantRecette' in column:
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (Programme) et la deuxième colonne (date)
    Programme = row[0] 
    ...
    test_data.append([Programme, date])

# Convertir test_data en tableau numpy
test_data = np.array(test_data)  # Nouvelles données : CodeWilaya, date qui représente le X_test
...
#     scene=dict(
#         xaxis_title='CodeWilaya',
#         yaxis_title='Date',
#         zaxis_title='Total Montant'
#     )
# )
...
#     plot_tree(regressor_tree, feature_names=['CodeWilaya', 'Date'], filled=True)
#     plt.show()

...
def format_predictions(test_data, y_pred):
    ...
    for i, prediction in enumerate(y_pred):
        ...
        prediction_str = f"codeISIN: {test_data[i][0]}, TotalMontantDette: {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'codeISIN', 'TotalMontantDette' et 'DATE'
    for column in columns:
        if 'codeISIN' in column:
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (codeISIN) et la deuxième colonne (date)
    codeISIN = row[0] 
    ...
    test_data.append([codeISIN, date])

# Convertir test_data en tableau numpy
test_data = np.array(test_data)  # Nouvelles données : codeISIN, date qui représente le X_test
...
#     plot_tree(regressor_tree, feature_names=['codeISIN', 'Date'], filled=True)
#     plt.show()

...
def format_predictions(test_data, y_pred, libelleSoumissionaire_values):
    predictions_list = []
    for i, prediction in enumerate(y_pred):
        days = int(test_data[i][1])
        prediction_str = f"CodeSoumissionaire: 00{test_data[i][0]}, libelleSoumissionaire: {libelleSoumissionaire_values[i]}, TotalMontantDette: {prediction}, DATE: {(datetime(1970, 1, 1) + timedelta(days=days)).strftime('%Y-%m-%d')}"
...
for line in lines:
    ...
    # Extraire les valeurs de colonne pour 'CodeSoumissionaire', 'TotalMontantDette' et 'DATE'
    for column in columns:
        if 'CodeSoumissionaire' in column:
            value = int(column.split(':')[1].strip())  # Extraire et convertir en entier
            column_values.append(value)
        elif 'TotalMontantDette' in column:
            value = int(float(column.split(':')[1].strip()))  # Convertir en entier
            column_values.append(value)
        elif 'libelleSoumissionaire' in column:
            value = column.split(':')[1].strip()  # Extraire la valeur de libelleSoumissionaire
            libelleSoumissionaire_values.append(value)
        elif 'DATE' in column:
...
# print(training_data)
# print("\nLibelle Soumissionaire Values:")
# for libelle in libelleSoumissionaire_values:
#     print(libelle)
...
for row in training_data:
    # Extraire les valeurs de la première colonne de training_data (CodeSoumissionaire) et la deuxième colonne (date)
    CodeSoumissionaire = row[0] 
    ...
    test_data.append([CodeSoumissionaire, date])

# Convertir test_data en tableau numpy
test_data = np.array(test_data)  # Nouvelles données : CodeSoumissionaire, date qui représente le X_test

# *******************************************Regression linéaire multiple*************************************************
# Construire le modèle c-a-d le nuage de points autrement dit la relation entre X et y (l'apprentissage par la machine)
...
# print("R^2 score for Polynomial Regression:", r3)
...
if max_r2 == r2:
    ...
    predictions_strings = format_predictions(test_data, y_pred,libelleSoumissionaire_values)
    ...
    # Utilisation de la fonction pour formater les prédictions
    predictions_strings = format_predictions(test_data, y_pred_poly,libelleSoumissionaire_values)
    # Affichage de la liste de prédictions formatées
    for prediction_str in predictions_strings:
        print(prediction_str)
    ...
    # Utilisation de la fonction pour formater les prédictions
    predictions_strings = format_predictions(test_data, y_tree_pred,libelleSoumissionaire_values)
    # Affichage de la liste de prédictions formatées
    for prediction_str in predictions_strings:
        print(prediction_str)
...
#     scene=dict(
#         xaxis_title='CodeSoumissionaire',
#         yaxis_title='Date',
#         zaxis_title='Total Montant Remboursé'
#     )
# )
...
#     plot_tree(regressor_tree, feature_names=['CodeSoumissionaire', 'Date'], filled=True)
#     plt.show()

# # Plot with zoom-out effect (smaller figure size)
# plot_tree_with_size(18, 15)  # Zoom-out



# # Plot with zoom-in effect (larger figure size)
# plot_tree_with_size(40, 30)  # Zoom-in
